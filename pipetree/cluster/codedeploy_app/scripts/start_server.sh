...
docker run -v /var/pipetree/:/var/pipetree/ --log-driver=awslogs --log-opt awslogs-region=us-west-1  --log-opt awslogs-group=pipetreeContainerLogs --log-opt awslogs-stream=${INSTANCE_TYPE}_${INSTANCE_ID} -t pipetree-user-image > /dev/null 2> /dev/null < /dev/null &
exit 0
